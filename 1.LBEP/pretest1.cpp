...
void Q2()
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        printf(" - ho ten: ");
        gets(ds[i].name);
        printf(" - diem khoa hoc: ");
        scanf("%d", &ds[i].science);
        printf(" - diem toan: ");
        scanf("%d", &ds[i].math);
    ...
    // vong lap xuat bang ds sinh vien thi dau
    printf("\n +++ Bang danh sach sinh vien thi dau +++ \n");
    int cntPass = 0;
    for (int i = 0; i < n; i++)
    {
        if (ds[i].science >= 40 && ds[i].math >= 40 && ds[i].english >= 40)
        {
            printf("\t ma so: %s\n", ds[i].id);
            printf("\t ho ten: %s\n", ds[i].name);
            printf("\t diem khoa hoc  : %3d\n", ds[i].science);
    ...
    if(cntPass ==0){
        printf(">> Ko tim thay sinh vien thi dau !!! \n");
    }
}